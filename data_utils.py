...
class SortedNumberGenerator(object):
    ...
    def __init__(self, batch_size, subset, terms, positive_samples=1, predict_terms=1, image_size=28, color=False, rescale=True, min_int=0, max_int=5000):
    ...
    def next(self):
        def consecutive_numbers(hist_num, future_num):
            ...
            result = np.array([array_n((start + i)%(self.max_int+1)) for i in range(hist_num + future_num)])

...
class SortedNumberGenerator(object):
    ...
    def __init__(self, batch_size, subset, terms, positive_samples=1, predict_terms=1, image_size=28, color=False, rescale=True):
        ...
        # Initialize MNIST dataset
        self.mnist_handler = MnistHandler()
        self.n_samples = self.mnist_handler.get_n_samples(subset) // terms  # a sample consists of a list (of length terms) of images
        self.n_batches = self.n_samples // batch_size
    ...
    def next(self):
        ...
        # Build sentences -- a sentence is a list of images
        image_labels = np.zeros((self.batch_size, self.terms + self.predict_terms)) # a batch of sentences
        sentence_labels = np.ones((self.batch_size, 1)).astype('int32') # label for sentences, positive if it's consecutive, negative otherwise
        positive_samples_n = self.positive_samples  # number of positive samples
        ...
        # Retrieve actual images according to image_labels (result is flattened)
        ...
        # Randomly shuffle items in the batch
...
class  numbSameNumberGenerator(object):

...
class Generator(Module):
    ...
    def _build_generator(self, x):
        ...
        x = self._dense_norm_activation(x, 7 * 7*3 * 128, activation=relu)
        x = tf.reshape(x, [-1, 7, 7*3, 128])
        x = tf.image.resize_images(x, (14, 14*3))
        x = self._conv_norm_activation(x, 128, 4)
        x = relu(bn(x))
        x = tf.image.resize_images(x, (28, 28*3))
...
class Critic(Module):
    ...
    def _build_critic(self, x):
        ...
        x = tf.image.pad_to_bounding_box(x, 0, 0, 8, 8*3)
        x = leaky_relu(bn(x))
        x = dropout(x)
